...
class PostManager extends Connexion
{
    public function getPosts($page, $post = NULL)
    {
        $bdd = $this->dbConnect();
        if (!empty($page)) {
            $listPosts = $bdd->prepare('
                                                    SELECT posts.id, posts.title, posts.kicker, users.pseudo, posts.content, posts.url, posts.created_at, posts.modified_at, categories.category, categories.id 
